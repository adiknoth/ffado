...
#define FW_DEVICE_PREFIX "/dev/fw"
int Ieee1394Service::getNewStackPort()
{
    if(m_portName.compare(0, strlen(FW_DEVICE_PREFIX), FW_DEVICE_PREFIX) == 0) {
        std::string portnumber = m_portName.substr(strlen(FW_DEVICE_PREFIX));
        debugOutput(DEBUG_LEVEL_VERBOSE,
                    "New stack port %s => %s\n",
                    m_portName.c_str(), portnumber.c_str());
        return atoi(portnumber.c_str());
    } else {
        debugWarning("Port is not a new stack port\n");
        return -1;
    }
}
...
Ieee1394Service::show()
{
    ...
    debugOutput( DEBUG_LEVEL_VERBOSE, " CycleTimerHelper: %p, WatchDog: %p\n",
                 m_pCTRHelper, m_pWatchdog );
