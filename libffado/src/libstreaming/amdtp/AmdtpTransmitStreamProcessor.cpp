    ...
    uint32_t ts )
{
    ...
    packet->syt = bswap_16 ( timestamp_SYT );
    ...
    for (; i < m_nb_audio_ports; i++) {
        ...
        if(p.buffer && p.enabled) {
            ...
            for(;j < nevents; j += 1) {
                ...
                *target_event = bswap_32((quadlet_t)tmp);
                buffer++;
                target_event += m_dimension;
            }
    ...
    for (; i < m_nb_audio_ports; i++) {
        ...
        if(p.buffer && p.enabled) {
            ...
            for(;j < nevents; j += 1) {
                ...
                *target_event = bswap_32((quadlet_t)((in & 0x00FFFFFF) | 0x40000000));
                buffer++;
                target_event += m_dimension;
            }
                                                     ...
                                                     unsigned int nevents)
{
    ...
    for (i = 0; i < m_nb_midi_ports; i++) {
        ...
        for (j = p.location;j < nevents; j += 8) {
            ...
            *target_event = bswap_32(IEC61883_AM824_SET_LABEL(0, IEC61883_AM824_LABEL_MIDI_NO_DATA));
        }
    }
}
