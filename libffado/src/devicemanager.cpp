...
#warning TODO: move this to the streaming layer
bool
DeviceManager::initStreaming(unsigned int samplerate)
{
        ...
        ++it )
    {
        ...
        if (!device->setSamplingFrequency(samplerate)) {
            debugOutput(DEBUG_LEVEL_VERBOSE,
...
bool
DeviceManager::startStreamingOnDevice(unsigned int i)
{
    if(i >= getAvDeviceCount()) {
        debugError("Device index out of range\n");
        return false;
    }

    FFADODevice *device = m_avDevices.at(i);
    assert(device);
...
bool
DeviceManager::stopStreamingOnDevice(unsigned int i)
{
    if(i >= getAvDeviceCount()) {
        debugError("Device index out of range\n");
        return false;
