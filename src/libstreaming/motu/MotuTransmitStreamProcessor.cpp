    ...
    unsigned char *data, unsigned int *length)
{
    ...
    if (getDebugLevel() > 0) {
        ...
        for (i=0; i<n_events; i++, sample+=m_event_size) {
            ...
            signed int val = bswap_32(lrintf(0x7fffff*sin((1000.0*2.0*M_PI/24576000.0)*a_cx)));
...
//fprintf(stderr,"tpf=%f\n", ticks_per_frame);
        for (int i=0; i < n_events; i++, quadlet += dbs) {
            ...
            *quadlet = bswap_32(fullTicksToSph(ts_frame));
//fprintf(stderr,"tx: %d/%d\n",
    ...
    uint32_t ts )
{
    ...
    // transmitted block.
    *quadlet = bswap_32(0x00000400 | ((m_Parent.get1394Service().getLocalNodeId()&0x3f)<<24) | m_tx_dbc | (dbs<<16));
    quadlet++;
    *quadlet = bswap_32(0x8222ffff);
    quadlet++;
    return n_events;
}
