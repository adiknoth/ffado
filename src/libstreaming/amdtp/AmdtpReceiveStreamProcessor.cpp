                                                 ...
                                                 uint32_t pkt_ctr)
{
    ...
    if(ok) {
        m_last_timestamp = sytRecvToFullTicks2((uint32_t)bswap_16(packet->syt), pkt_ctr);
        //#ifdef DEBUG
        #if 0
        uint32_t now = m_1394service.getCycleTimer();

        //=> convert the SYT to a full timestamp in ticks
        uint64_t old_last_timestamp = sytRecvToFullTicks((uint32_t)bswap_16(packet->syt),
    ...
    for (i = 0; i < m_nb_audio_ports; i++) {
        ...
        if(p.buffer && p.enabled) {
            ...
            for(j = 0; j < nevents; j += 1) {
                *(buffer)=(bswap_32((*target_event) ) & 0x00FFFFFF);
                buffer++;
                                                    ...
                                                    unsigned int nevents)
{
    ...
    for (i = 0; i < m_nb_audio_ports; i++) {
        ...
        if(p.buffer && p.enabled) {
            ...
            for(j = 0; j < nevents; j += 1) {
                unsigned int v = bswap_32(*target_event) & 0x00FFFFFF;
    ...
    for (i = 0; i < m_nb_midi_ports; i++) {
        ...
        if (p.buffer && p.enabled) {
            ...
            for (j = p.location;j < nevents; j += 8) {
                ...
                sample_int=bswap_32(*target_event);
