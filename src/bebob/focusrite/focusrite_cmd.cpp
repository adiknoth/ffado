...
FocusriteVendorDependentCmd::serialize( Util::Cmd::IOSSerialize& se )
{
    ...
    // FIXME: this is not consistent, we should not have to care about bswap_32 here
    result &= se.write(bswap_32(m_id),"FocusriteVendorDependentCmd ID");
    result &= se.write(bswap_32(m_value),"FocusriteVendorDependentCmd value");
